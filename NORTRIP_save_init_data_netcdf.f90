...
subroutine NORTRIP_create_init_netcdf(filename)
    ...
    call check(nf90_def_dim(ncid,"num_track", 1 , trackid))
    call check(nf90_def_dim(ncid,"num_size", 4 , sizeid))
    ...
    call check(nf90_def_var(ncid, "M_road_data", nf90_double, (/sourceid,sizeid,trackid,roadid/),varid))
    call check(nf90_put_att(ncid,varid,"description","Mass data array"))

    call check(nf90_def_var(ncid, "road_meteo_data", nf90_double, (/varnumid,trackid,roadid/),varid))
    call check(nf90_put_att(ncid,varid,"description","road meteo data array"))
    call check(nf90_def_var(ncid, "g_road_data", nf90_double, (/moistid,trackid,roadid/),varid))
    call check(nf90_put_att(ncid,varid,"description","g_road_data array"))

    call check(nf90_def_var(ncid = ncid, name = "time_since_last_salting", xtype=nf90_double,dimids=roadid, varid=varid))
    call check(nf90_def_var(ncid = ncid, name = "time_since_last_binding", xtype=nf90_double,dimids=roadid, varid=varid))
    call check(nf90_def_var(ncid = ncid, name = "time_since_last_sanding", xtype=nf90_double,dimids=roadid, varid=varid))
    call check(nf90_def_var(ncid = ncid, name = "time_since_last_cleaning", xtype=nf90_double,dimids=roadid, varid=varid))
    call check(nf90_def_var(ncid = ncid, name = "time_since_last_ploughing", xtype=nf90_double,dimids=roadid, varid=varid))
...
subroutine NORTRIP_save_init_data_netcdf
    ...
    if (hour_test.eq.0.or.ti.eq.max_time) then
        ...
        if (save_road_data_flag(ro).ne.0) then !TODO: why is the flag a function of ro?
            ...
            !TODO: No formating is done except defining the netcdf variables as nf90_double. Should be put on approproate format when the file is read.
